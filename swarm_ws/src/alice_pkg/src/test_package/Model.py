...
class Model:
    ...
    def generateIdeal(self):
        ...
        while ideal == None and not rospy.is_shutdown():
            ...
            for rule in ideal_tuple:
                if rule != None:
                    ideal = (rule[0], rule[1], 0, (float(len(ideal_tuple) - ideal_tuple.index(rule) + 1))/(tolerance + 1), 	self.name) #The +1 is so that priority is never zero, which would mess up VectorQueue
                    self.current = rule[2]
    ...
    def addRobot(self, to_add): #Should be of the form (direction, distance, heading)
        self.robots.append((to_add.theta, to_add.distance, to_add.heading))
    ...
    def clear(self):
	    self.speed = 0
	    self.obstacles = [] #Saved in form (Direction, distance)
	    self.robots = [] #Saved in form (Direction, distance, speed, angle)
	    self.food = [] #Not implemented, but will probably be stored as (Direction, distance)
	    self.rules = Rules()
	    self.current = ""
	    self.tolerance = 0 
