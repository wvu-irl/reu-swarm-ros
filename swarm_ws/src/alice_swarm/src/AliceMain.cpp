...
void checkForUpdatedChargers()
{
	...
	else
	{
		...
		{
			if (prev_chargers.charger.at(i).occupied
					!= temp_mail.abs_chargers.at(i).occupied)
			{
				same = false;
			}
			i++;
...
int main(int argc, char **argv)
{
	...
	ros::Publisher pub = n.advertise < wvu_swarm_std_msgs::robot_command_array
			> ("final_execute", 1000);
	ros::Publisher pub2 = n.advertise < wvu_swarm_std_msgs::map
			> ("info_map", 1000);
	ros::Publisher pub3 = n.advertise < wvu_swarm_std_msgs::chargers
			> ("chargers", 1000);
	ros::Publisher pub4 = n.advertise < wvu_swarm_std_msgs::priorities
			> ("priority", 1000);
	ros::ServiceClient client = n.serviceClient < alice_swarm::get_maps
			> ("get_maps");
	...
	{
#if DEBUG
		ROS_INFO("Checking for charger updates");
#endif
		...
		{
#if DEBUG
			ROS_INFO("Generating command [%d]", i);
#endif
			//==gives each robot the relative data it needs, whilst also creating the alice's
			if (temp_mail.mails.size() > i && temp_priorities.priorities.size() > i)
			{
				alice_map[temp_mail.mails.at(i).name].updateModel(temp_mail.mails.at(i),
						maps, ids, &temp_mail.abs_chargers,
						temp_priorities.priorities.at(i).priority);

#if DEBUG
				ROS_INFO("[%d] sent relative data", i);
#endif

				wvu_swarm_std_msgs::robot_command temp;
				AliceStructs::vel tempVel =
						alice_map[temp_mail.mails.at(i).name].generateVel();

#if DEBUG
				ROS_INFO("[%d] generated vel", i);
#endif
...
#if DEBUG
				ROS_INFO("[%d] done creating command", i);
#endif
			...
			{
				ROS_WARN("[%d] Not doing stuff: %d && %d", i,
						temp_mail.mails.size() > i, temp_priorities.priorities.size() > i);
			}
#endif
		}
...
//		std::cout<<"number of bot priors: "<<temp_priorities.priorities.size()<<std::endl;
		...
		{
			priorities_to_publish.priorities.push_back(
					temp_priorities.priorities.at(i));
		}
