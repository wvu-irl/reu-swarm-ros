...
//		std::cout<<"x,y: "<<x<<","<<y<<"\n";
	...
	// gets vicon input for mixed sim and real
		ros::Subscriber sub6= _n.subscribe("real_locations_array", 1000, &Sim::realBotCallback, this);
	ros::Rate loopRate(20);

	//publishes initial information for each bot
	wvu_swarm_std_msgs::vicon_bot_array vb_array = flock.createMessages(real_bots);
...
PrevIteration Sim::HandleInput(PrevIteration _pI)	//handles input to the graphics window
{
	...
	while (window.pollEvent(event))
	{
		...
		pause(event.type == sf::Event::KeyPressed, event.key.code == sf::Keyboard::Space, pauseSim, &window, event);
...
void Sim::clickNdragObstacles(PrevIteration *_pI, float _mX, float _mY, sf::Event _event) //for click and drag on bots
{
			...
			&& _pI->prevClick == false && _pI->obs == false && _pI->bot == false && _pI->target == false)
	{
		while (found != true && i < obstacles.point.size())
		{
			...
			if (x_pos > _mX - 3 && x_pos < _mX + 3 && y_pos > _mY - 3 && y_pos < _mY + 3)
			{
				found = true;
				_pI->botId = i;
				_pI->dragging = true;
