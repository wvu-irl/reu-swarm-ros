...
const sf::String instructions[] = {"Select TOP LEFT", "Select TOP RIGHT", "Select BOTTOM RIGHT", "Select BOTTOM LEFT", "Press space to write or R to reset"};
static int state = 0;
...
#if DEBUG
    std::cout << "Created vertex array size : " << (state + 1 + (state == 3 ? 1 : 0)) << std::endl;
#endif
    switch (state)
    {
    default:
...
void changeInstruction(sf::String str)
{
    ...
    instruct.setPosition(sf::Vector2f(WIDTH / 2 - instruct.getLocalBounds().width / 2, HEIGHT / 2));
}
...
int main(int argc, char **argv)
{
    ...
    fon.loadFromFile(sf::String("ComicSansMS3.ttf"));
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
            ...
            std::cout << "Mouse moved Event" << std::endl;
                ...
                {
                case 0:
                    g_calib.top_left = sf::Vector2f(event.mouseMove.x, event.mouseMove.y);
                    break;
                case 1:
                    g_calib.top_right = sf::Vector2f(event.mouseMove.x, event.mouseMove.y);
                    break;
                case 2:
                    g_calib.bottom_right = sf::Vector2f(event.mouseMove.x, event.mouseMove.y);
                    break;
                case 3:
                    g_calib.bottom_left = sf::Vector2f(event.mouseMove.x, event.mouseMove.y);
                    break;
                default:
                    break;
                }
            ...
            case sf::Event::KeyPressed:
                ...
                else if (state == 4 && event.key.code == sf::Keyboard::Space)
                {
                    ...
                    file << g_calib.top_right.x << "," << g_calib.top_right.y << std::endl;
                    file << g_calib.bottom_right.x << "," << g_calib.bottom_right.y << std::endl;
                    file << g_calib.bottom_left.x << "," << g_calib.bottom_left.y << std::endl;
                    file.close();
