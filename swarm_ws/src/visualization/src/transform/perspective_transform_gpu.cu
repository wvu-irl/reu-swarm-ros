...
{
	vector2f_t top = {scale(initial.x, 0, width, trap.tl.x, trap.tr.x),
			scale(initial.x, 0, width, trap.tl.y, trap.tr.y)};
	vector2f_t bottom = {scale(initial.x, 0, width, trap.bl.x, trap.br.x),
			scale(initial.x, 0, width, trap.bl.y, trap.br.y)};
	vector2f_t left = {scale(initial.y, 0, height, trap.bl.x, trap.tl.x),
			scale(initial.y, 0, height, trap.bl.y, trap.tl.y)};
	vector2f_t right = {scale(initial.y, 0, height, trap.br.x, trap.tr.x),
...
__global__ void transform(sf::Uint8 *cols_in, sf::Uint8 *cols_out, size_t *width, size_t *height, quadrilateral_t *trap)
{
	...
	{
		int col_ind = i * 4;
		int o_col_ind = (trap_point.y * *width + trap_point.x) * 4;
...
void perspectiveTransform(quadrilateral_t trap, sf::RenderTexture rt, sf::Uint8 *tf_cols)
{
	...
	cudaMemcpyAsync(tf_cols, d_cols_out, 4 * sizeof(sf::Uint8) * width * height, cudaMemcpyDeviceToHost, 0);
	...
	checkCudaErrors(cudaFree(d_cols_in));
	checkCudaErrors(cudaFree(d_cols_out));
