...
void sendCommandToRobots(command cmd, int recip_rid)
{
  ...
  {
    if (sockets->at(i).getRID() == -2 || sockets->at(i).getRID() == recip_rid || recip_rid == -1) // selecting who is to be send the command
...
void *runClient(void *args)
{
  ...
  std::function<bool()> exit_condition_callback = vals->exit_condition_callback;
  int id = vals->id;
  ...
  while (exit_condition_callback()) // running until told to stop
  {
    ...
    //reading the message
    while ((message_size = read(connection_descriptor, buffer, sizeof(command))) > 0)
    {
  ...
  while (exit_condition_callback())
  {
    // Specify struct to store information about accepted connection
      ...
      else
      {
        ...
        struct client_param clinet_args = (struct client_param){command_callback, info_callback, error_callback, exit_condition_callback, (int)sockets->size() - 1};
